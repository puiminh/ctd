...
Token* getToken(void) {
  ...
  case CHAR_PLUS: 
    readChar();
    if (charCodes[currentChar]==CHAR_PLUS){
      readChar();
      return makeToken(SB_INC, lineNo, colNo);
    }
    else{
  ...
  case CHAR_MINUS:
    readChar();
    if (charCodes[currentChar]==CHAR_MINUS){
      readChar();
      return makeToken(SB_DEC, lineNo, colNo);
    }
    else{
      token = makeToken(SB_DEC, lineNo, colNo); 
    }
  case CHAR_TIMES:
